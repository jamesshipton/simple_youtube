...
describe 'YoutubeUpdateSpec' do
  ...
  it "updates a video on youtube" do
    ...
    video_uid = "wOzOc0xxJu8"
    user_name = "vinylpusherman"
    ...
    access_token.should_receive(:put).with("http://gdata.youtube.com/feeds/api/users/#{user_name}/uploads/#{video_uid}", update_xml, {'Accept' => 'application/atom+xml', 'Content-Type' => 'application/atom+xml', 'X-GData-Key' => x_gdata_key})

    Youtube::Video.update(video_uid, user_name, update_xml, oauth_token, oauth_token_secret, x_gdata_key, host, host_secret)
